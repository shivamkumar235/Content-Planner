    ...
    >
          ...
          <span
            className={
              Theam == "dark"
                ...
                : " flex items-center w-fit gap-1 border-3 border-[#e0ded8] rounded-2xl px-3 py-[0.1vw] bg-[#f5f4f0] text-[#5f5e5a]"
            }
          >
      ...
      >
        ...
        <div className="flex gap-7">
          ...
          <RiEditLine
            onClick={() => {
              localStorage.setItem("editVideo", JSON.stringify(propes));
              localStorage.setItem("editIndex", propes.index);
              navigate("/Editcontent");
            }}
            className="text-[#a1a1aa] hover:text-cyan-500 "
          />
          {/* </Link> */}
          <RiDeleteBin6Line
            className="text-[#a1a1aa] hover:text-red-500"
            onClick={() => deleteVideo(propes.idx)}
          />
        </div>
      </div>
    </div>
